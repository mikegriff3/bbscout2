...
import CPlayerRatings from "./CPlayerRatings";
import CPlayerSeasonStats from "./CPlayerSeasonStats";
import CPlayerFullStats from "./CPlayerFullStats";
//import PlayerCareerStats from "./PlayerCareerStats";
import CRankGauges from "./player-charts/CRankGauges";
import COffBar from "./player-charts/COffBar";
import CDefBar from "./player-charts/CDefBar";
...
class CPlayerInfo extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      teamStats: [],
  ...
  renderSelection() {
    if (this.state.selection === "Player Ratings") {
      return <CPlayerRatings player={this.state.player} />;
      ...
      return <CPlayerSeasonStats player={this.state.player} />;
  ...
  getOffenseRating() {
    if (this.state.player) {
      let obpm = parseFloat(this.state.player.obpm);
      let obpmRating = (100 / 18) * (obpm - 9.5) + 100;
      let ows = parseFloat(this.state.player.ows);
      var owsRating = (100 / 6.3) * (ows - 5.0) + 100;
      let offRating = obpmRating * 0.5 + owsRating * 0.5;
      let grade = this.getGrade(100, 0, offRating);
      if (grade.Grade === 80) {
        return (
          <button
            style={{
  ...
  getDefenseRating() {
    if (this.state.player) {
      let dbpm = parseFloat(this.state.player.dbpm);
      let dbpmRating = (100 / 8.0) * (dbpm - 6) + 100;
      let dws = parseFloat(this.state.player.dws);
      let dwsRating = (100 / 3.0) * (dws - 2.5) + 100;
      let defRating = dbpmRating * 0.5 + dwsRating * 0.5;
      let grade = this.getGrade(100, 0, defRating);
      if (grade.Grade === 80) {
        return (
          <button
            style={{
  ...
  getOverallRating() {
    if (this.state.player) {
      let per = parseFloat(this.state.player.per);
      let perRating = (100 / 30) * (per - 30) + 100;
      let bpm = parseFloat(this.state.player.bpm);
      let bpmRating = (100 / 17) * (bpm - 12) + 100;
      let ws = parseFloat(this.state.player.ws);
      let wsRating = (100 / 6.5) * (ws - 6.5) + 100;
      let weightedOvr = perRating * 0.5 + bpmRating * 0.3 + wsRating * 0.2;
      let grade = this.getGrade(100, 0, weightedOvr);
      if (grade.Grade === 80) {
        return (
          <button
            style={{
  ...
  renderBarChart() {
    if (this.state.barSelect === "Offense") {
      return <COffBar player={this.state.player} />;
    } else if (this.state.barSelect === "Defense") {
      return <CDefBar player={this.state.player} />;
  ...
  checkLoad() {
    ...
    let drafted = this.state.player.draft;
    if (drafted === "Undrafted Undrafted") drafted = "Undrafted";
        ...
        <div className="container-fluid">
              ...
              <div className="player-info">
                  ...
                  <span className="player__info-stat-title">
                    ...
                    <span className="player__info-stat-text">
                      <a
                        ...
                        href={`/gleague-team/${this.state.colors.id}`}
                      >
                ...
                <div className="player__info-text">
                  <span className="player__info-stat-title">
                    High School:{" "}
                    <span className="player__info-stat-text">
                      {this.state.player.highschool || "None"}
                    </span>
                  </span>
                </div>
                <div className="player__info-text">
                  <span className="player__info-stat-title">
                    Hometown:{" "}
                    <span className="player__info-stat-text">
                      {this.state.player.hometown || "Unavailable"}
                    </span>
                  </span>
                </div>
              </div>
          ...
          <div className="row" style={{ backgroundColor: "rgba(0,0,0,0.7)" }}>
            ...
            >
              ...
              <CPlayerFullStats seasons={this.state.stats} />
            </div>
          </div>
          <div
            className="hr-box"
...
export default connect(mapStateToProps, mapDispatchToProps)(CPlayerInfo);
