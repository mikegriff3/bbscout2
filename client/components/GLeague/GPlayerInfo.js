...
import GPlayerSeasonStats from "./GPlayerSeasonStats";
//import PlayerCareerStats from "./PlayerCareerStats";
import GRankGauges from "./player-charts/GRankGauges";
import GOffBar from "./player-charts/GOffBar";
import GDefBar from "./player-charts/GDefBar";
//import OvrBar from "./player-charts/OvrBar";
//import CareerProgression from "./player-charts/CareerProgression";
import GPlayerFullStats from "./GPlayerFullStats";
...
class GPlayerInfo extends React.Component {
  ...
  renderSelection() {
    if (this.state.selection === "Player Ratings") {
      return <GPlayerRatings player={this.state.player} />;
      ...
      return <GPlayerSeasonStats player={this.state.player} />;
  ...
  getOffenseRating() {
    if (this.state.player) {
      var ORtg = parseFloat(this.state.player.obpm);
      var offRating = (100 / 38) * (ORtg - 118) + 100;
      var ows = parseFloat(this.state.player.ows);
      var owsRating = (100 / 4) * (ows - 2.5) + 100;
      let ovrOff = offRating * 0.8 + owsRating * 0.2;
      let grade = this.getGrade(97, 0, ovrOff);
      if (grade.Grade === 80) {
        return (
          <button
            style={{
  ...
  getDefenseRating() {
    if (this.state.player) {
      var DRtg = parseFloat(this.state.player.dbpm) * -1;
      var defRating = (100 / 30) * (DRtg + 90) + 100;
      var dws = parseFloat(this.state.player.dws);
      var dwsRating = (100 / 2.2) * (dws - 2.2) + 100;
      let ovrDef = defRating * 0.8 + dwsRating * 0.2;
      let grade = this.getGrade(80, 0, ovrDef);
      if (grade.Grade === 80) {
        return (
          <button
            style={{
  ...
  getOverallRating() {
    if (this.state.player) {
      ...
      var DRtg = parseFloat(this.state.player.dbpm) * -1;
      var defRating = (100 / 30) * (DRtg + 90) + 100;
      var pie = parseFloat(this.state.player.plusMinus * 100);
      var pieRating = (100 / 20) * (pie - 20) + 100;
      var per = parseFloat(this.state.player.per);
      var perRating = (100 / 30) * (per - 30) + 100;
      // var ws = parseFloat(this.state.player.ws);
      // var wsRating = (100 / 4.7) * (ws - 4.0) + 100;
      let ovrRating =
        perRating * 0.4 + pieRating * 0.4 + offRating * 0.1 + defRating * 0.1;
      let grade = this.getGrade(90, 0, ovrRating);
      if (grade.Grade === 80) {
        return (
          <button
            style={{
  ...
  renderBarChart() {
    if (this.state.barSelect === "Offense") {
      return <GOffBar player={this.state.player} />;
    } else if (this.state.barSelect === "Defense") {
      return <GDefBar player={this.state.player} />;
        ...
        <div className="container-fluid">
            ...
            <div className="player-stats col-sm-8 col-xs-12">
              ...
              >
                ...
                >
                  ...
                  <div className="stat-box">
                    <span className="stat-title">PIE</span>
                    <span className="stat-text">
                      {(this.state.player.plusMinus * 100).toFixed(1) || 0}
                    </span>
                  </div>
                </div>
                <div
          ...
          <div className="row" style={{ backgroundColor: "rgba(0,0,0,0.7)" }}>
            ...
            >
              ...
              <GPlayerFullStats seasons={this.state.stats} />
            </div>
          </div>
          <div
            className="hr-box"
