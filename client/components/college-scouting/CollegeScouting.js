...
import CPlayerScatter from "./college-scouting-charts/CPlayerScatter";
...
export default class CollegeScouting extends React.Component {
  ...
  getOverallRating(player) {
    let per = parseFloat(player.per);
    let perRating = (100 / 30) * (per - 30) + 100;
    let bpm = parseFloat(player.bpm);
    let bpmRating = (100 / 17) * (bpm - 12) + 100;
    let ws = parseFloat(player.ws);
    let wsRating = (100 / 6.5) * (ws - 6.5) + 100;
    let weightedOvr = perRating * 0.5 + bpmRating * 0.3 + wsRating * 0.2;
    return this.getGrade(100, 0, weightedOvr);
  }

  getOffensiveRating(player) {
    let obpm = parseFloat(player.obpm);
    let obpmRating = (100 / 18) * (obpm - 9.5) + 100;
    let ows = parseFloat(player.ows);
    var owsRating = (100 / 6.3) * (ows - 5.0) + 100;
    let offRating = obpmRating * 0.5 + owsRating * 0.5;
    return this.getGrade(100, 0, offRating);
  }

  getDefensiveRating(player) {
    let dbpm = parseFloat(player.dbpm);
    let dbpmRating = (100 / 8.0) * (dbpm - 6) + 100;
    let dws = parseFloat(player.dws);
    var dwsRating = (100 / 3.0) * (dws - 2.5) + 100;
    let defRating = dbpmRating * 0.5 + dwsRating * 0.5;
  ...
  calculateOffGrades(player) {
    ...
    grades.pts = this.getGrade(highPoints, 4, (player.pts / player.mpg) * 32);
    grades.ast = this.getGrade(highAst, 0, (player.ast / player.mpg) * 32);
    grades.reb = this.getGrade(highReb, 1, (player.trb / player.mpg) * 32);
    ...
    grades.orb = this.getGrade(highOrb, 0, (player.orb / player.mpg) * 32);
    grades.tov = this.getGrade(
      highTov,
      -4.0,
      ((player.tov * -1) / player.mpg) * 32
    );
    grades.twoPointPct = this.getGrade(highTwo, 0.35, player.twoPtPct);
    grades.fgPct = this.getGrade(highFgPct, 0.32, player.fgPct);
  ...
  calculateDefGrades(player) {
    ...
    grades.drb = this.getGrade(highDrb, 1, (player.drb / player.mpg) * 32);
    grades.stl = this.getGrade(highStl, 0, (player.stl / player.mpg) * 32);
    grades.blk = this.getGrade(highBlk, 0, (player.blk / player.mpg) * 32);
    grades.pf = this.getGrade(highPf, -4, (player.pf / player.mpg) * 32 * -1);
    ...
    if (this.state.pfArr.length === 0) {
      ...
      return this.state.pfArr.map((player, index) => (
        <div className="row" style={{ display: "flex", marginBottom: "20px" }}>
          ...
          >
            <a href={`/college-player/${player.id}`}>{player.name}</a>
  ...
  filterpfPlayers(players, state) {
    ...
    if (this.state.oStat1 === "twoPtPct") {
      if (this.state.offg1OU === "gte") {
        players = players.filter(function (player) {
          return player.offStatGrades.twoPointPct >= state.oGrade1;
        });
      } else {
        players = players.filter(function (player) {
          return player.offStatGrades.twoPointPct <= state.oGrade1;
    ...
    if (this.state.oStat2 === "twoPtPct") {
      if (this.state.offg2OU === "gte") {
        players = players.filter(function (player) {
          return player.offStatGrades.twoPointPct >= state.oGrade2;
        });
      } else {
        players = players.filter(function (player) {
          return player.offStatGrades.twoPointPct <= state.oGrade2;
    ...
    if (this.state.oStat3 === "twoPtPct") {
      if (this.state.offg3OU === "gte") {
        players = players.filter(function (player) {
          return player.offStatGrades.twoPointPct >= state.oGrade3;
        });
      } else {
        players = players.filter(function (player) {
          return player.offStatGrades.twoPointPct <= state.oGrade3;
    ...
    if (!this.state.pfEXP1) {
      players = players.filter(function (player) {
        return player.class !== "FR";
      });
    }
    if (!this.state.pfEXP2) {
      players = players.filter(function (player) {
        return player.class !== "SO";
      });
    }
    if (!this.state.pfEXP3) {
      players = players.filter(function (player) {
        return player.class !== "JR";
      });
    }
    if (!this.state.pfEXP4) {
      players = players.filter(function (player) {
        return player.class !== "SR";
      });
    }
              ...
              <div className="row" style={{ display: "flex" }}>
                <div className="col-sm-3">
                  ...
                  <form className="filter-form-pf">
                    <input
                      type="checkbox"
                      checked={this.state.pfG}
                      onChange={this.handlepfPG}
                      id="pos1"
                      name="pos1"
                      value="PG"
                    />
                    <label for="pos1"> G</label>
                    <br />
                    {/*<input
                      type="checkbox"
                    ...
                    <input
                      type="checkbox"
                      checked={this.state.pfF}
                      onChange={this.handlepfSF}
                      id="pos3"
                      name="pos3"
                      value="F"
                    />
                    <label for="pos3"> F</label>
                    <br />
                    {/*<input
                      type="checkbox"
            ...
            <div className="col-sm-3">
              ...
              <form className="filter-form">
                ...
                <input
                  ...
                  disabled
                />
                <label for="exp2"> Sophomore</label>
                <br />
                <input
                  type="checkbox"
  ...
  handlepfPG(evt) {
    this.setState({ pfG: evt.target.checked });
  }
  ...
  handlepfSF(evt) {
    this.setState({ pfF: evt.target.checked });
  }
