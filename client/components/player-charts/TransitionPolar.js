...
export default class TransitionPolar extends React.Component {
  constructor() {
    super();
  ...
  calculateGrades() {
    ...
    var freq = this.getGrade(highFreq, this.state.player.freq * 100, 3.2);
    ...
    var efg = this.getGrade(highEfg, this.state.player.efg * 100, 29.0);
    var scoreFreq = this.getGrade(
      highScoreFreq,
      this.state.player.scoreFreq * 100,
      20.0
    );
    var toFreq = this.getGrade(
      highToFreq,
      this.state.player.toFreq * -1 * 100,
      -37.0
    );
    var ftFreq = this.getGrade(highFtFreq, this.state.player.ftFreq * 100, 0);
      ...
      series: [
        {
          ...
          data: [
            {
              y: this.state.freq.Grade - 6,
              temp: this.state.freq.Grade,
              color: this.state.freq.Color,
              borderColor: this.state.freq.ColorStroke,
              borderWidth: 3,
              name: "Freq%",
              stat: (this.state.player.freq * 100).toFixed(1),
            },
            {
              y: this.state.ppp.Grade - 6,
              temp: this.state.ppp.Grade,
            ...
            },
            {
              ...
              borderWidth: 3,
              name: "eFG%",
              stat: (this.state.player.efg * 100).toFixed(1),
            },
            {
              y: this.state.scoreFreq.Grade - 6,
              temp: this.state.scoreFreq.Grade,
              color: this.state.scoreFreq.Color,
              borderColor: this.state.scoreFreq.ColorStroke,
              borderWidth: 3,
              name: "Score Freq.",
              stat: (this.state.player.scoreFreq * 100).toFixed(1),
            },
            {
              y: this.state.toFreq.Grade - 6,
              temp: this.state.toFreq.Grade,
              color: this.state.toFreq.Color,
              borderColor: this.state.toFreq.ColorStroke,
              borderWidth: 3,
              name: "TOV Freq.",
              stat: this.state.player.toFreq * 100,
            },
            {
              y: this.state.ftFreq.Grade - 6,
              temp: this.state.ftFreq.Grade,
  ...
  render() {
    return (
      <div>
        <div id="container-trans" style={{ height: "30rem", width: "3" }} />
      </div>
    );
  }
}
